    ...
    async def run(
        ...
        while iteration < max_turn:
            ...
            response = await self._generate_step(
                agents['main'], sampling_params, request_id, metrics
            )
            ...
            if agents['main'].chat[-1]['role'] == 'user':
                logger.warning('[ROLE ERROR] Appending empty assistant turn')
                agents['main'].chat.append({'role': 'assistant', 'content': str(response)})
                agents['main']._generation_prompt = None
                turn_tokens = agents['main']._get_turn_tokens(len(agents['main'].chat) - 1)
                agents['main'].chat_ids.append(turn_tokens)
                agents['main'].log_probs.append([0.0] * len(turn_tokens))
                agents['main'].token_mask.append([False] * len(turn_tokens))
                agents['main'].additional_info.append(None)
            ...
            agents['main'].append_user(observation)
        ...
        for name in agent_names_to_output:
            ...
            # Determine reward for this agent
